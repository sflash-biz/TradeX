...
$(function()
{
    ...
    var cookie_network = $.cookie(COOKIE_NAME_NETWORK) != null ? $.cookie(COOKIE_NAME_NETWORK).split('|') : null;
    ...
    .tablesorter({
        ...
        sortList: cookie_network != null ? [[cookie_network[0],cookie_network[1]]] : null
    })
    .bind('sortColumn', function(e, sortColumn, sortDirection)
    {
        $.cookie(COOKIE_NAME_NETWORK, sortColumn + '|' + sortDirection, {expires: 365});
    });
    ...
    $('img[src="images/edit-32x32.png"]')
        .click(function()
        {
            ...
            $.ajax({
                async: true,
                data: 'r=_xNetworkSitesBulkEditShow&domain=' + selected.join(',')
            });
        });
...
function processUnknowns()
{
    ...
    else
    {
        ...
        // Load total stats
        if (owner) owner = '&owner=' + owner; else owner = '';
        if (category) category = '&category=' + category; else category = '';
        $.ajax({
            async: true,
            data: 'r=_xNetworkStatsTotalGet' + owner + category,
            success: function(data)
            {
...
function updateStatsRow($tr, response, total)
{
    ...
    $tr.children('td:nth-child('+(i++)+')').html(number_format(response.i_se_raw_24));
}
