...
$(function()
{
    ...
    .click(function()
    {
        ...
        $('tbody td:first-child input:checkbox', $table).each(function() {
            this.checked = $('thead input.check-all').is(':checked') ? true : false;
        });
    });

    //////////////////////////
    // Inverse all checkbox by right-click
    $('thead input.check-all')
    ...
    .click(function()
    {
        if( $(this).is(':checked') )
        {
            ...
            {
                $('thead input.check-all').prop('checked', true);
            }
        }
        else
        {
            $('thead input.check-all').prop('checked', false);
        }
    });
});
...
(function($)
{
    ...
    function initializeCalendar()
    {
        ...
        html_buffer += '</select> <span id="cal_ok_btn">ok</span> </td></tr>' +
                       '</table>' +
                       '</div>';
    ...
    function updateCalendar()
    {
        ...
        $('#cal #cal_ok_btn').click(okBtnClicked);
    ...
    // A OK btn was clicked on
    function okBtnClicked()
    {
        var now = new Date();
        var time = $('#cal_hour').val() + ':' + $('#cal_minute').val() + ':' + $('#cal_second').val();
        var matches = $('#cal tr.numbers > td.selected').attr('id').match(/(\d\d\d\d)-(\d\d)-(\d\d)/);
        var datetime = matches[1] + '-' + matches[2] + '-' + matches[3] + ($('tr.time:visible').length ? ' ' + time : '');
