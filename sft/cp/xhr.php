...
///////////////////////////////////////
function _xRedisTest()
{
    global $C;

    $redis_host_port = explode(':', $_REQUEST['redis_host_port']);
    //Check required fields
    if (!empty($redis_host_port[0]))
    {
        $C['redis_host'] = $redis_host_port[0];
    }
    else
    {
    ...
    if( $C['redis_work'] == 1 )
    {
        JSON::Success('The Redis host:port appears to be valid!');
    }
    elseif ($C['redis_work'] == 2)
    {
        JSON::Success('The Redis socket appears to be valid!');
    }
    elseif ($C['redis_work'] == 3)
    {
        JSON::Warning('Looks like you try using Redis host with socket!');
    }
    elseif ($C['redis_work'] == 4)
    {
        JSON::Warning('Can not connect to Redis!');
    }
    else
    {
    ...
    if( $trade['flag_grabber'] )
    {
        ...
        switch($thumbnails)
        {
            ...
            default:
                ...
                {
                    $html .= '<img src="../th/' . $trade['domain'] . '-' . $i . '.jpg?' . mt_rand() . '">';
                }
...
function _xUpdateExtractInstaller()
{
    ...
    /* save lic key in tmp file */
    $lic_key_file = DIR_DATA . '/lic.dat';
    file_write($lic_key_file, LIC_KEY, 0666);
        ...
        switch($type)
        {
            case ITEM_TYPE_DIR:
                ...
                if (!file_exists($name))
                {
                    ...
                    if ($cd)
                    {
                        ...
                        file_append(FILE_LOG_UPDATE, "[Done]\n");
                    }
                    else
                    {
                        file_append(FILE_LOG_UPDATE, "[Error]: cant create\n");
                    }
                }
                else
                {
                    file_append(FILE_LOG_UPDATE, "[Warning]: $name already exist\n");
                }
                break;
            ...
            case ITEM_TYPE_FILE:
                if( $on_patch == 1 || (!file_exists($name) && $on_patch == 2) )
                {
                    ...
                    {
                        file_append(FILE_LOG_UPDATE, "\tAppend to file $name - ");
                    }
                    ...
                    if ($fp_out)
                    {
                        ...
                        {
                            file_append(FILE_LOG_UPDATE, "[Error]: file base64_decode\n");
                        }
                        fclose($fp_out);
                        @chmod($name, $permissions);
                    }
                    ...
                    {
                        file_append(FILE_LOG_UPDATE, "[Error]: cant read dir or create file\n");
                    }
                }
                break;
        }
    ...
    // Copy out.php to new place
    if (realpath(DIR_BASE . '/out.php') != realpath($C['out_path']))
    {
        $result = copy(DIR_BASE . '/out.php', $C['out_path']);
        file_append(FILE_LOG_UPDATE, $result ? "\tCopying out.php - [Done]\n" : "\t[Error]: cant copy out.php to {$C['out_path']}\n");
    }
    else
    {
        file_append(FILE_LOG_UPDATE, "\tNo need to copy out.php - [Done]\n");
    }




    /* add licence key to utility.php */
    define('FILE_UTILITY', DIR_BASE . '/lib/utility.php');
    $utility = string_format_lf(file_get_contents(FILE_UTILITY));
    $utility = str_replace('/*#<LIC_KEY>*/', file_get_contents($lic_key_file), $utility);
    $result = file_write(FILE_UTILITY, $utility, 0644);
    file_append(FILE_LOG_UPDATE, $result ? "\tAdd licence key to utility.php - [Done]\n" : "\t[Error]: cant add licence key to utility.php\n");
    file_delete($lic_key_file);
    ...
    $domains = array();
    $params = array();
    $engines = file_get_contents(FILE_SEARCH_ENGINES);
    ...
    $uas = array();
    $spiders = file_get_contents(FILE_SPIDERS);
    foreach( explode(STRING_LF_UNIX, $spiders) as $line )
    {
        $uas[] = $line;
    }
    $in = preg_replace('~/\*#<SPIDERS>\*/.*?/\*#</SPIDERS>\*/~',
        '/*#<SPIDERS>*/' . (count($uas) ? "'" . join("','", $uas) . "'" : '') . '/*#</SPIDERS>*/',
        $in);

    $result = file_write(FILE_IN_PHP, $in, 0666);
    file_append(FILE_LOG_UPDATE, $result ? "\tUpdate in.php " . '(/*#<ENGINES>*/, /*#<ENGINE_PARAMS>*/, /*#<SPIDERS>*/)' . " - [Done]\n" : "\t[Error]: cant update in.php " . '(/*#<ENGINES>*/, /*#<ENGINE_PARAMS>*/, /*#<SPIDERS>*/)' . "\n");



    /* Update the out.php file */
    $out = file_get_contents(FILE_OUT_PHP);

    $out = preg_replace('~/\*#<SPIDERS>\*/.*?/\*#</SPIDERS>\*/~',
        '/*#<SPIDERS>*/' . (count($uas) ? "'" . join("','", $uas) . "'" : '') . '/*#</SPIDERS>*/',
        $out);

    $outlist_weights = explode('|', file_get_contents(FILE_OUTLIST_WEIGHTS));
    $out = preg_replace('~/\*#<OUTLIST_POINTS>\*/.*?/\*#</OUTLIST_POINTS>\*/~',
        '/*#<OUTLIST_POINTS>*/' . (count($outlist_weights) ? '' . join(',', $outlist_weights) . '' : '') . '/*#</OUTLIST_POINTS>*/',
        $out);

    $result = file_write(FILE_OUT_PHP, $out, 0666);
...
function _xNetworkSiteDelete()
{
    ...
    $settings = array();
    $to_delete = explode(',', $_REQUEST['settings']);



    if( in_array(NETWORK_DELETE_TRADES, $to_delete) )
    {
        $trades = explode(',', $_REQUEST['trades']);
        ...
        $settings[NETWORK_DELETE_TRADES] = array();
        ...
        foreach( $trades as $trade )
        {
            ...
            if( !empty($trade) )
            {
                $settings[NETWORK_DELETE_TRADES][] = $trade;
            }
        }
    }
...
function _xTradesExport()
{
    ...
    foreach( $db->RetrieveAll() as $trade )
    {
        ...
        $lines[] = preg_replace_callback('~\{\$([a-z_]+)\}~', function($m) use ($trade) { return $trade[$m[1]]; }, $_REQUEST['format']);
    }
...
function _xTradesBulkAdd()
{
    ...
    foreach( explode(STRING_LF_UNIX, string_format_lf($_REQUEST['data'])) as $line )
    {
        ...
        $data['force_instant_owed'] = $_REQUEST['force_instant'];

        if( empty($_REQUEST['force_hourly']) )
        {
            $data['force_hourly_end'] = '';
            $data['force_hourly_hours'] = '';
        }
        else
        {
            if (!empty($_REQUEST['force_hourly_hours'])) {
                $add_hours = (Int)$_REQUEST['force_hourly_hours'];
                $data['force_hourly_end'] = add_hours_to_time($add_hours);
            }
        }
...
function _xTradesAdd()
{
    ...
    {
        $_REQUEST['force_instant_owed'] = (Int)$_REQUEST['force_instant'];
    }

    if( empty($_REQUEST['force_hourly']) )
    {
        $_REQUEST['force_hourly_end'] = '';
        $_REQUEST['force_hourly_hours'] = '';
    }
    else
    {
        if (!empty($_REQUEST['force_hourly_hours'])) {
            $add_hours = (Int)$_REQUEST['force_hourly_hours'];
            $_REQUEST['force_hourly_end'] = add_hours_to_time($add_hours);
        }
    }
...
function _xTradesBulkEdit()
{
    ...
    if( $_REQUEST['flag_update']['force_hourly_end'] ) $_REQUEST['flag_update']['force_hourly_hour'] = true;
    ...
    if( $_REQUEST['flag_update']['force_hourly_end'] )
    {
        if (!empty($_REQUEST['force_hourly_hours'])) {
            $add_hours = (Int)$_REQUEST['force_hourly_hours'];
            $_REQUEST['force_hourly_end'] = add_hours_to_time($add_hours);
...
function _xGlobalSettingsSave()
{
    ...
    //$_REQUEST['out_path'] analyze
    if (stripos($_REQUEST['out_path'], DIR_SITE_ROOT) !== 0) {
        $_REQUEST['out_path'] = DIR_SITE_ROOT . '/' . trim($_REQUEST['out_path'], '/');
    }
    ...
    $v->Register($_REQUEST['out_path'], VT_FILE_IS_WRITEABLE, 'The ' . $_REQUEST['out_path'] . ' script must have 666 permissions');
...
{
    $v =& Validator::Get();
    $v->Register(FILE_OUTLIST_WEIGHTS, VT_FILE_IS_WRITEABLE, 'The ' . FILE_OUTLIST_WEIGHTS . ' file has incorrect permissions; change them to 666');
    $v->Register(FILE_OUT_PHP, VT_FILE_IS_WRITEABLE, 'The ' . FILE_OUT_PHP . ' script must have 666 permissions');
    ...
    foreach ($_REQUEST as $key => $request)
    {
        if (strpos($key, 'weight_') === 0) {
            if ((Int)$request !== 0) $outlist_weights[] = (Int)$request;
        }
    }
    ...
    // Update the outlist_weights file
    file_write(FILE_OUTLIST_WEIGHTS, join('|', $outlist_weights));
...
function _xSpidersSave()
{
    ...
    $uas = array();
    $spiders = string_remove_blank_lines(str_replace("'", '', $_REQUEST['spiders']), false);
    ...
    // Update the spiders file
    file_write(FILE_SPIDERS, $spiders);
