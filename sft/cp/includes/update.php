...
?>
    ...
    <div class="message-error ta-center" style="width: 900px; margin-left: auto; margin-right: auto;">
      Unable to connect to update host for update:
    ...
    <div id="no-new-version-message" class="message-warning d-none ta-center">No new version of SFTrade is available at this time</div>
    <div id="new-version-message" class="message-notice d-none ta-center">A new version of SFTrade is available!</div>
...
{
    $('#update-progress > span').text('Downloading installer from update host...');
    $('#update-progress').show();

    $.ajax({
        async: true,
        data: 'r=_xUpdateGetInstaller&version=' + encodeURI($('select[name="version"]').val()),
...
function extractInstaller()
{
    ...
    $.ajax({
        async: true,
        data: 'r=_xUpdateExtractInstaller&su=' + encodeURI($('input[name="su"]').is(':checked') ? 1 : 0),
        success: function(data)
        {
            if( data[JSON.KEY_STATUS] == JSON.STATUS_SUCCESS )
            {
