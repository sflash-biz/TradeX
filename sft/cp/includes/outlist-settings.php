    ...
    <form method="post" action="xhr.php" class="xhr-form">
        ...
        <div id="dialog-panel">
            <div style="padding-top: 2px;">
                ...
                $OUTLIST_WEIGHTS_FIELDS_MIN = 15;

                $main_outlist = load_outlist(FILE_OUTLIST_MAIN);
                $g_counter = 1;
                $g_color = '#ececec';
                $outlist_weights = explode('|', file_get_contents(FILE_OUTLIST_WEIGHTS));
                $OUTLIST_WEIGHTS_FIELDS = count($outlist_weights) > $OUTLIST_WEIGHTS_FIELDS_MIN ? count($outlist_weights) : $OUTLIST_WEIGHTS_FIELDS_MIN;
        ...
        <div id="dialog-buttons">
            ...
            <input type="button" id="button-spot-plus" value=" + ">
            <input type="button" id="button-spot-minus" value=" - ">
            <input type="submit" id="button-save" value="Save" style="margin-left: 10px;">
...
{
?>
<table id="outlist-settings-table" class="item-table no-wrap fsize-9pt" border="0" cellspacing="0" cellpadding="4" align="center" style="width: 465px; min-width: 465px;">
    <thead>
    <tr>
    ...
    function _outlist_table_row_main($trade, $percent, $points)
    {
        ...
        ?>
        <tr bgcolor="<?php echo $g_color; ?>" id="outlist-table-row-<?php echo $g_counter; ?>">
    ...
    <script>
        ...
        var fields_amount = <?=$OUTLIST_WEIGHTS_FIELDS;?>;
        var fields_amount_min = <?=$OUTLIST_WEIGHTS_FIELDS_MIN;?>;
        ...
        function _outlist_table_row_main(g_counter) {
            return '<tr bgcolor="#d9d9d9" id="outlist-table-row-' + g_counter + '">' +
                '<td class="ta-right" style="padding-right: 4px;">' +
                g_counter +
                '</td>' +
                '<td class="ta-right" style="padding-right: 4px;">' +
                '<span><input type="text" size="5" value="" name="weight_' + g_counter + '" maxlength="5" pattern="[0-9]+"></span>' +
                '</td>' +
                '<td style="padding: 0;" class="va-middle">' +
                '<div class="percent-bar ta-left" style="width: 0;" id="percent-bar-' + g_counter + '">' +
                '<span id="percent-val-"' + g_counter + '>0.00%</span>' +
                '</div>' +
                '<span class="va-middle" id="percent-val-' + g_counter + '"></span>' +
                '</td>' +
                '<td class="ta-right"></td>' +
                '</tr>';
        }

        var outlist_settings_table_body = $("#outlist-settings-table tbody");
        var c_line = fields_amount;

        $('#button-spot-plus').click(function() {
            c_line++;
            outlist_settings_table_body.append(_outlist_table_row_main(c_line));
        });

        $('#button-spot-minus').click(function() {
            if (c_line > fields_amount_min) {
                $('#outlist-settings-table tbody tr:last').remove();
                c_line--;
            }
        });
    </script>
