...
<script type="text/javascript" src="js/stats-overall.js"></script>
    ...
    <table id="system-stats" class="item-table no-wrap" border="0" cellspacing="0" cellpadding="4" align="center" style="min-width: 1000px;">
      ...
      {
          if (empty($sh->trade['flag_exlude_global'])) $sh_system->AddStats($sh);
          _stats_hourly_table_row($sh, 'system-action-menu', true);
      }
      ?>
      </tbody>
      <tfoot>
      <?php
      _stats_hourly_table_row($sh_system, null, true);
      unset($system_stats);

      $sh_total = new StatsHourly('Grand Total');
      $sh_total->AddStats($sh_trades);
      $sh_total->AddStats($sh_system);
      _stats_hourly_table_row($sh_total, null, true);
      ?>
      </tfoot>
    </table>
    ...
    <div id="toolbar">
      <div id="toolbar-content">
        <div id="toolbar-icons">
          ...
          <img src="images/toolbar-separator-2x32.png">
          <a href="http://sflash.biz/sft-dist-pub/docs/stats-hourly.html" title="Documentation" target="_blank"><img src="images/help-32x32.png" border="0"></a>
        </div>
      </div>
    </div>
...
function _stats_hourly_table_row($sh, $menu = 'trade-action-menu', $system = false)
{
    global $current_hour;
...
?>
        ...
        <tr <?php if( !empty($sh->trade['color']) ) echo 'style="background-color: ' . $sh->trade['color'] . ';"'; ?> id="item-<?php echo str_replace(' ', '_', $domain); ?>" class="ta-right<?php if (!empty($sh->trade['flag_exlude_global'])) echo ' light-font-color'; ?>">
          ...
          <td class="va-middle" style="padding-right: 4px;"<?php if( empty($menu) ): ?> colspan="2"<?php endif; ?>>
            ...
            ?>
            <?php if( $system ) { ?>
            <img src="images/edit-16x16.png" class="trade-edit" fnc="_xSystemTradesEditShow">
