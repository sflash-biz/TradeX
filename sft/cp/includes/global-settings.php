        ...
        <div id="dialog-panel" dwidth="900px">
            <div style="padding-top: 2px;">
                ...
                <fieldset>
                    ...
                    <div class="field">
                        <label>Cookie Session Name:</label>
                        <span><input name="cookie_tdxsess" value="<?php echo $C['cookie_tdxsess']; ?>" size="30" type="text"></span>
                    </div>

                    <div class="field">
                        <label>Cookie Sig Name:</label>
                        <span><input name="cookie_tdxsig" value="<?php echo $C['cookie_tdxsig']; ?>" size="30" type="text"></span>
                    </div>

                    <div class="field">
                        <label>Cookie Bookmark Name:</label>
                        <span><input name="cookie_tdxbookmark" value="<?php echo $C['cookie_tdxbookmark']; ?>" size="30" type="text"></span>
                    </div>
                ...
                <fieldset>
                    ...
                    <select name="storage_method">
                      <option value="File system"<?=($C['storage_method'] == 'File system') ? ' selected="selected"' : ''?>">File system</option>
                        <option value="Redis"<?=($C['storage_method'] == 'Redis') ? ' selected="selected"' : ''?>>Redis</option>
                    </select>
                    <img src="images/test-16x16.png" title="Test Redis" class="c-click" id="test-redis">
                  </span>
                    </div>

                    <div class="field" style="display: none;">
                        <label>Redis Host or Socket:</label>
                        <span><input type="text" class="storage-field storage-redis" size="20" value="<?php echo $C['redis_host']; ?>" name="redis_host"></span>
                    </div>

                    <div class="field" style="display: none;">
                        <label>Redis Port (or "sock"):</label>
                        <span><input type="text" class="storage-field storage-redis" size="20" value="<?php echo $C['redis_port']; ?>" name="redis_port"></span>
                    </div>
                </fieldset>
...
<script language="JavaScript" type="text/javascript">
    ...
    $('#dialog-panel')
        .bind('dialog-visible', function()
        {
            ...
            $('select[name="storage_method"]')
                .change(function()
                {
                    var storage = $(this).val().toLowerCase();
                    $('.storage-field').parents('div.field').hide();
                    $('.storage-' + storage).parents('div.field').show();

                    //Show hide redis checker
                    if ($('.storage-' + storage).parents('div.field')[0])
                        $('#test-redis').show();
                    else
                        $('#test-redis').hide();
                })
    ...
    $('#test-mogrify')
        .click(function()
        {
            ...
            $.ajax({
                async: true,
                data: 'r=_xMogrifyTest&magick_mogrify_path=' + encodeURI($('input[name="magick_mogrify_path"]').val()),
                complete: function()
                {
    ...
    $('#test-redis')
        .click(function()
        {
            ...
            $.ajax({
                async: true,
                data: 'r=_xRedisTest&redis_host_port=' + encodeURI($('input[name="redis_host"]').val() + ':' + $('input[name="redis_port"]').val()),
                complete: function()
                {
                    $icon.attr('src', 'images/test-16x16.png');
                }
