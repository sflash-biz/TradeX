...
$C['out_path'] = (!empty($C['out_path'])) ? $C['out_path'] : DIR_BASE . '/out.php';
define('FILE_OUT_PHP', $C['out_path']);
...
define('FILE_OUTLIST_WEIGHTS', DIR_DATA . '/outlist_weights');
define('FILE_SPIDERS', DIR_DATA . '/spiders');
...
function write_outlist($file, $sort_key, &$points, $amount = null)
{
    ...
    // For empty trades random out
    $points = shuffle_assoc($points);

    uasort($points, 'cmp_outlist_points');

//    $points_tmp = $points;
//    if (!empty($amount)) $points_tmp = array_slice($points_tmp, 0, $amount);
    ...
    foreach( $points as $trade => $data )
    {
        ...
        // Again check trader to <= 0 (fixed to < 0)
...
{
    global $C, $g_stats_hourly, $g_stats_daily;
    ...
    foreach( stats_get_all_files() as $stats_file )
    {
        ...
        $system_count = true;
        if ($system) {
            $file = DIR_SYSTEM . '/' . basename($stats_file);
            $fp = fopen($file, 'r');
            $line = trim(fgets($fp));
            $sts = explode('|', $line);
            if (!empty($sts[6])) $system_count = false;  // system trade have enabled settings "Exclude from global stats"
        }
    ...
    if ($C['storage_method'] == 'File system') delete_old_sessions();
}
...
function get_dns($domain)
{
    ...
    if (!shell_command_exist('dig')) return;
...
function get_config_defaults()
{
    ...
    $defaults['traffic_url'] = 'http://www.' . ltrim($_SERVER['HTTP_HOST'], 'www.') . '/';
    $defaults['cms_url'] = 'http://www.prn3.com/';
    $defaults['out_path'] = 'sft/out.php';
    $defaults['base_url'] = 'http://www.' . ltrim($_SERVER['HTTP_HOST'], 'www.') . preg_replace('~/cp/.*~', '', $_SERVER['REQUEST_URI']);
    $defaults['cookie_domain'] = $domain;
    $defaults['cookie_path'] = preg_replace('~/cp/.*~', '', $_SERVER['REQUEST_URI']);
    $defaults['cookie_tdxsess'] = 'sftsecoc';
    $defaults['cookie_tdxsig'] = 'sftsicoc';
    $defaults['cookie_tdxbookmark'] = 'sftbookcoc';
    $defaults['keyphrase'] = md5(uniqid(mt_rand(), true));
    $defaults['storage_method'] = 'File system';
    ...
    $defaults['flag_accept_new_trades'] = '0';
    ...
    $defaults['magick_mogrify_path'] = 'mogrify';
    $defaults['magick_mogrify_options'] = '+profile "*" -format jpg -quality 80';
    $defaults['thumb_resize_width'] = '240';
    $defaults['thumb_resize_height'] = '180';
    $defaults['thumb_trigger_strings'] = 'thumbs/,tn/,thumbnails/,th/';
    $defaults['trades_satisfied_url'] = 'http://www.porns.nu/';
...
function get_all_trades_logs($log = 'in')
{
    ...
    $system_trades_ = dir_read_files(DIR_SYSTEM);
    $system_trades = array();
    foreach( $system_trades_ as $i => $trade )
    {
        $file = DIR_SYSTEM . '/' . $trade;
        $fp = fopen($file, 'r');
        $line = trim(fgets($fp));
        $sts = explode('|', $line);
        if (!empty($sts[6])) continue;  // system trade have enabled settings "Exclude from global stats"

        $system_trades[] = DIR_SYSTEM_STATS . "/$trade-$log";
    }
...
function dir_read($directory, $pattern, $type, $sort = true)
{
    ...
    {
        trigger_error('File not found'.$directory, E_USER_ERROR);
    }

    if( !is_dir($directory) )
    {
        trigger_error('Not a directory'.$directory, E_USER_ERROR);
    }
...
{
    //exec('find ' . DIR_BASE . '/data/sessions -type f -mmin +60 -exec rm {} \;');
    exec('find ' . DIR_BASE . '/data/sessions -type f -mmin +60 -exec rm {} \; ; find ' . DIR_BASE . '/data/sessions -not -path ' . DIR_BASE . '/data/sessions -type d -empty -delete');
}

function delete_old_cp_sessions()
{
    exec('find ' . DIR_BASE . '/data/cp_sessions -type f -mmin +1440 -exec rm {} \;');
}

function add_hours_to_time($add_hours, $format="Y-m-d H:i:s")
{
    return date($format, strtotime("+{$add_hours} hours"));
}
////////////////////////////
