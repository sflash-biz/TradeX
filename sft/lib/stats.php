...
//        if (is_system_trade($trade['domain'])) {
//            $file = DIR_SYSTEM . '/' . $trade['domain'];
//            $fp = fopen($file, 'r');
//            $line = trim(fgets($fp));
//            $sts = explode('|', $line);
...
//        }
        ...
        for( $i = 0; $i < MINUTES_PER_HOUR; $i++ )
        {
            ...
            {
                $rt60[$j] += $k;
                if ($trade['domain'] == 'search-engine' && ($j == 1 || $j == 2)) { //TODO
                    $se24[$j] += $k;
                }
            }
        ...
        $trade_stats = new StatsOverall($trade, $rt60, $rt24, $se24);
...
function load_site_stats()
{
    ...
    return new StatsOverall($trade, $rt60, $rt24, $se24); //TODO
}
...
class StatsOverall
{
    ...
    function __construct($trade, $stats60 = null, $stats24 = null, $se_stats24 = null)
    {
        ...
        if( !empty($se_stats24) )
        {
            ...
            // SE In last 24 hours
            $this->i_se_raw_24 = $se_stats24[$i++];
            $this->i_se_uniq_24 = $se_stats24[$i++];
        }
    }
    ...
    function AddStats($s)
    {
        ...
        // Return Instant Forces Owed
        $this->trade['force_instant_owed'] += $s->trade['force_instant_owed'];

        // SE In last 24 hours
        $this->i_se_raw_24 += $s->i_se_raw_24;
        $this->i_se_uniq_24 += $s->i_se_uniq_24;
    ...
    function SetUnknown()
    {
        ...
        // In SE last 24 hours
        $this->i_se_raw_24 = STATS_UNKNOWN;
        $this->i_se_uniq_24 = STATS_UNKNOWN;
...
class StatsDetailed
{
    ...
    // Browser detection
/*    var $browser_regex = '~(MSIE \d+\.\d+|Trident/\d+\.\d+|Firefox/\d+\.\d+|Chrome/\d+\.\d+|Opera/\d+\.\d+|iPhone OS|Android \d+\.\d+|Version/\d+\.\d+|Safari/\d+|Blackberry\d+|Konqueror/\d+\.\d+)~i';
    var $browser_search = array( 'MSIE',              'rv:',               'Version/', 'Firefox/', 'Opera/', 'Safari/', 'Chrome/', 'Konqueror/');
    var $browser_replace = array('Internet Explorer', 'Internet Explorer', 'Safari ',  'Firefox ', 'Opera ', 'Safari ', 'Chrome ', 'Konqueror ');*/

    var $browser_regex = '~999999999999999999999999999999999999999999999999999999999~i';
    var $browser_search = array();
    var $browser_replace = array();
